...
# 【全局变量】记录当前选择的LLM方法，决定是用RAG还是其他大模型
# 可选值：'直接回复 Direct Reply'、'金融RAG系统'、'Qwen'、'ChatGPT'等
DEFAULT_LLM_METHOD = '金融RAG系统'
current_llm_method = DEFAULT_LLM_METHOD
...
@calculate_time
def LLM_response(
    question_audio, question,  # 输入的音频（Whisper识别后的）和文本问题
    voice, rate, volume, pitch,  # 语音合成参数：声音、语速、音量、音调
    am, voc, lang, male,  # TTS模型参数：声学模型、声码器、语言、性别
    ...
    # ========== 第1步：生成回答 ==========
    # 根据用户选择的LLM方法，决定如何生成回答
    # 如果选了"金融RAG系统"，就通过HTTP请求调用RAG后端
    # 否则用Linly-Talker内置的LLM（如Qwen、ChatGPT等）
    ```python
    # 定义全局变量，标识当前选用的大模型工作模式
    global current_llm_method
    # 判断当前运行模式是否为【金融RAG检索增强问答系统】模式
    if current_llm_method == '金融RAG系统':
        try:
            # 导入网络请求库，用于向后端RAG服务发送HTTP接口调用
            import requests
            # ====================== 核心RAG接口调用逻辑 ======================
            # 功能说明：通过POST网络请求访问独立部署的金融RAG后端服务
            # 服务部署地址：本机本地服务 localhost:8000
            # 接口类型：POST接口 /api/v1/chat 对话接口
            # 接口入参说明：
            # 1. message：用户输入的原始提问内容，是大模型需要处理的核心问题
            # 2. user_id：用户唯一标识，这里固定0，用于区分会话、保存历史对话上下文
            # 3. use_knowledge：布尔开关，True=开启知识库检索，False=仅调用大模型不检索文档
            # 超时设置60秒原因：RAG完整链路耗时较长，包含3个阶段
            # ①向量库文档检索 ②检索内容拼接提示词 ③大模型生成回答，整体速度慢于普通LLM
            resp = requests.post(
                "http://localhost:8000/api/v1/chat",
                json={"message": question, "user_id": 0, "use_knowledge": True},
                timeout=60  # 请求超时阈值，超过60秒未收到响应直接抛出超时异常
            )
            # 判断HTTP响应状态码，200代表接口请求成功、服务正常返回数据
            if resp.status_code == 200:
                # 将后端返回的JSON字符串转换成Python字典，方便提取字段
                data = resp.json()
                # 从返回字典中读取key为answer的字段，即RAG生成的完整回答
                # 兜底逻辑：如果后端返回数据缺少answer字段，则直接把用户原问题作为回复避免空值报错
                answer = data.get("answer", question)
            else:
                # 接口请求成功连接，但服务内部处理出错（4xx参数错误/5xx服务崩溃等）
                # 打印错误状态码，便于定位后端问题
                print(f"RAG API返回错误: {resp.status_code}")
                # 异常兜底：接口异常时，回复内容赋值为用户原始提问，保证程序不会中断
                answer = question
            # 控制台打印RAG系统生成的最终回答，用于调试、日志查看
            print("金融RAG回复：", answer)
        except Exception as e:
            # 捕获调用RAG过程中所有类型异常，包含但不限于：
            # 1. ConnectionError：RAG后端服务未启动、端口8000未开放、网络连接失败
            # 2. Timeout：请求超过60秒未收到响应，触发超时
            # 3. JSONDecodeError：后端返回数据格式不是标准JSON，解析失败
            # 4. ImportError：requests库未安装，导入失败
            print(f"RAG调用失败: {e}，使用直接回复")
            # 异常兜底策略：RAG服务不可用时，将用户问题直接作为输出，避免流程卡死
            answer = question
    else:
        # 非RAG分支：使用Linly-Talker项目内置原生大模型推理
        # 不启动独立RAG后端，不检索金融知识库，仅依靠大模型基础能力回答问题
        # llm.generate：内置LLM推理函数
        # 参数1 question：用户提问文本
        # 参数2 DEFAULT_SYSTEM：全局预设系统提示词，定义大模型角色、回答规范、输出格式
        answer = llm.generate(question, DEFAULT_SYSTEM)
        # 控制台打印原生大模型的生成结果，用于调试日志输出
        print("LLM 回复：", answer)
    ```

    # ========== 第2步：TTS语音合成 ==========
    # 把大模型生成的文字回答转成语音wav文件
...
def webui_setting(talk=False):
    ...
    # 【UI组件】LLM模型下拉菜单
    # 默认选中"金融RAG系统"，这是本项目的核心功能
    # 用户可以切换到其他LLM（如Qwen、ChatGPT）进行对比
    llm_method = gr.Dropdown(
        choices=['Qwen', 'Qwen2', 'Linly', 'Gemini', 'ChatGLM', 'ChatGPT', 'GPT4Free', 'QAnything', '直接回复 Direct Reply', '金融RAG系统', 'Comming Soon!!!'],
        value=DEFAULT_LLM_METHOD,  # 默认值：使用金融RAG系统
        label='LLM 模型选择'
    )
...
def llm_model_change(model_name, progress=gr.Progress(track_tqdm=True)):
    ...
    try:
        ...
        elif model_name == '金融RAG系统':
            # 选择"金融RAG系统"时，不需要加载本地LLM模型
            # 因为实际的大模型调用在RAG后端完成（通过HTTP请求调用DeepSeek API）
            # 这里用"直接回复"作为占位符，真正的回答由LLM_response函数中的HTTP请求生成
...
if __name__ == "__main__":
    ...
    llm = llm_class.init_model('直接回复 Direct Reply')
    success_print(f"默认LLM模式为 {DEFAULT_LLM_METHOD}")
