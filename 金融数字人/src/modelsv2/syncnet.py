...
class SyncNet_color(nn.Module):
    """音视频同步性判断网络 (SyncNet, V2版本)
    
    SyncNet用于判断音频特征和视频面部特征是否同步（即嘴型与声音是否匹配）。
    它分别编码音频和面部图像特征，然后通过L2归一化后的向量计算相似度。
    
    V2版本的网络结构与V1版本基本一致，包含两个分支：
    1. 面部编码器(face_encoder): 处理多帧面部图像（15通道输入，
       即3帧RGB图像拼接9通道 + 额外6通道特征）
    2. 音频编码器(audio_encoder): 处理梅尔频谱图音频特征
    
    两个编码器的输出都被展平为512维向量并L2归一化，
    用于后续的余弦相似度计算以判断音视频同步性。
    """
    def __init__(self):
