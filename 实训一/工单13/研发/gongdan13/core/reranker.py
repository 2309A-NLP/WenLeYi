...
class Reranker:
    ...
    def _get_model(self):
        ...
        if self._model is None:
            ...
            print(f"[RERANKER] 模型加载完成(含import), 耗时={load_elapsed:.2f}s, 模型={self.model_path}")
        return self._model

    def warmup(self) -> Dict[str, float]:
        """提前加载模型并跑一次最小预测，避免首个用户请求承担推理初始化。"""
        timings = {}

        t0 = time.time()
        model = self._get_model()
        timings["model"] = round(time.time() - t0, 3)

        t0 = time.time()
        model.predict([("warmup", "warmup text")])
        timings["predict"] = round(time.time() - t0, 3)

        print(f"[RERANKER] warmup 完成: {timings}")
        return timings
