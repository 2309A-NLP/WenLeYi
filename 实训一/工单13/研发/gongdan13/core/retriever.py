...
class HybridRetriever:
    ...
    def _graph_search(self, query: str, limit: int = 5) -> List[Tuple[Dict, float]]:
        ...
        Returns:
            [(chunk, score), ...] 图谱检索结果，score 默认为 0.5
        """
        ...
        try:
            ...
            for e in entities:
                # 将三元组组合为可读文本
                text = f"{e.get('source', '')} {e.get('relation', '')} {e.get('target', '')}".strip()
                if text:
                    results.append(({"text": text, "source": "neo4j", "type": "graph"}, 0.5))
            return results
    ...
    def _weighted_search(self, query: str, top_k: int) -> List[Tuple[Dict, float]]:
        ...
        # --- 向量搜索在主线程执行（gRPC不支持跨线程）---
        print(f"[RETRIEVER] 加权检索启动: vec_top={faiss_k}, "
              f"bm25_top={bm25_k}, graph_top={graph_k}")

        # 向量搜索直接在主线程跑（gRPC线程安全问题）
        try:
            vec_results = self.vector_store.search(query, faiss_k)
        except Exception as exc:
            print(f"[RETRIEVER] 向量检索异常: {exc}")
        ...
        for chunk, score in bm25_norm:
            key = chunk["text"][:100]
            all_scores[key] = all_scores.get(key, 0) + score * bm25_weight
            all_chunks_map[key] = chunk

        for chunk, score in vec_norm:
            key = chunk["text"][:100]
            all_scores[key] = all_scores.get(key, 0) + score * vec_weight
            all_chunks_map[key] = chunk

        for chunk, score in graph_norm:
            key = chunk["text"][:100]
            all_scores[key] = all_scores.get(key, 0) + score * graph_weight
            all_chunks_map[key] = chunk
