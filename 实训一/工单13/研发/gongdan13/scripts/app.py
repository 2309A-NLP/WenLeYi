...
try:
    warmup_stats = vector_store.warmup("公司的组织架构是什么？")
    print(f"[APP] Embedding 模型预热完成, 耗时={_time.time() - _warmup_start:.1f}s, stats={warmup_stats}")
except Exception as e:
    print(f"[APP] Embedding 模型预热失败: {e}")
...
if reranker:
    ...
    try:
        ...
        print(f"[APP] Reranker 模型预热完成, 耗时={_time.time() - _warmup_start:.1f}s, stats={warmup_stats}")
    except Exception as e:
        print(f"[APP] Reranker 模型预热失败: {e}")
...
def api_query():
    ...
    return jsonify({"answer": answer, "sources": sources, "retrieve_time": retrieve_time})
...
def api_query_stream():
    ...
    def generate():
        ...
        # 在[DONE]消息中包含检索时间
        retrieve_time = rag._last_retrieve_time
        yield f"data: {json.dumps({'done': True, 'retrieve_time': retrieve_time}, ensure_ascii=False)}\n\n"
        yield "data: [DONE]\n\n"
