...
# 系统提示词：指导 LLM 如何基于参考资料和知识图谱回答问题
SYSTEM_PROMPT = """你是一个专业的金融文档问答助手。请根据提供的参考资料和知识图谱信息回答用户的问题。

要求：
1. 充分利用所有参考资料中的信息，尽可能全面、详细地回答问题
2. 如果资料中包含知识图谱（实体关系三元组），请结合图谱中的实体关系进行推理
3. 即使资料只涉及问题的部分方面，也要把能找到的信息都列出来
4. 如果某个子问题在资料中没有明确答案，可以基于相关背景知识进行合理推断，但要注明是推断
5. 只有当资料中完全没有与问题相关的内容时，才说"未找到相关信息"
6. 用阿拉伯数字分点输出（1. 2. 3.），条理清晰
7. 每点展开说明，包含具体数据和细节，不要过于简略
8. 不要使用任何 Markdown 格式符号（不要加 **、##、---、`、> 等）
9. 每个要点尽量标注信息来源（如：根据XX年报、根据XX数据等）
10. 对于分析类问题，要先列出事实依据，再给出分析结论"""


# 图谱上下文提示词：引导 LLM 利用知识图谱信息
GRAPH_CONTEXT_PROMPT = """【知识图谱信息】
以下是与问题相关的实体关系信息（来自知识图谱）：
{graph_context}

请结合上述知识图谱中的实体关系，以及参考资料，综合回答问题。
图谱中的关系可以帮助你理解实体之间的联系，发现隐含的信息。"""
    ...
    def query(self, question: str, use_history: bool = False, stream: bool = False, session_id: str = "default"):
        ...
        if use_history and self.config.ENABLE_MULTI_TURN:
            ...
            if self._should_rewrite_query(question, history):
                ...
                print(f"[RAG] Query 改写耗时={time.time() - rewrite_start:.2f}s", flush=True)

        # 1. 检索相关文档
        print("[RAG] 正在检索相关文档 ...", flush=True)
        results = self.retriever.retrieve(retrieval_question)
        print(f"[RAG] 检索返回 {len(results)} 条结果", flush=True)
        ...
        if self.reranker and self.config.ENABLE_RERANKER and results:
            ...
            print(f"[RAG] Reranker 精排完成, 共 {len(results)} 条", flush=True)
        ...
        # 4. 构建 prompt（将检索到的文档拼接为参考资料）
        # 分离文档 chunks 和图谱 chunks
        doc_chunks = [(c, s) for c, s in filtered if c.get("type") != "graph"]
        graph_chunks = [(c, s) for c, s in filtered if c.get("type") == "graph"]
        ...
        if graph_chunks:
            ...
            for chunk, _ in graph_chunks:
                text = chunk.get("text", "").strip()
                if text:
                    graph_lines.append(f"- {text}")
            if graph_lines:
                graph_context = "\n".join(graph_lines[:20])  # 限制图谱条目数
        ...
        if graph_context:
            prompt_parts.append(GRAPH_CONTEXT_PROMPT.format(graph_context=graph_context))
        prompt_parts.append(f"问题：{prompt_question}")

        prompt = "\n\n".join(prompt_parts)

        total_context_len = len(doc_context) + len(graph_context)
        print(f"[RAG] Prompt 构建完成: 文档上下文={len(doc_context)}字符, "
              f"图谱条目={len(graph_chunks)}条, 总上下文={total_context_len}字符", flush=True)
        ...
        if stream:
            ...
            def stream_gen():
                ...
                print(f"[RAG] LLM 调用完成 (stream), 耗时={elapsed:.2f}s, 回答长度={len(full_answer)}", flush=True)
            ...
            print(f"[RAG] LLM 调用完成 (chat), 耗时={llm_elapsed:.2f}s, 回答长度={len(answer)}", flush=True)
